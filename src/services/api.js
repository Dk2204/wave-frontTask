...
export const authAPI = {
  login: (username, password) => {
    ...
    const token = `demo-${btoa(username)}`;
  ...
  getCurrentUser: () => {
    ...
    try {
      const username = atob(token.replace('demo-', ''));
      const users = JSON.parse(localStorage.getItem('iz_users') || '[]');
...
export const subscriptionsAPI = {
  getUserSubscriptions: async () => {
    try {
      ...
      console.warn('Using mock subscriptions');
...
export const newsAPI = {
  getCompanyNews: async (params) => {
    try {
      ...
      const rawNews = response.data?.news || response.data || [];
      return rawNews.map(augmentNewsItem);
    } catch (error) {
      console.warn('Using mock news');
      const mockNews = MOCK_NEWS.news || [];
      return mockNews.map(augmentNewsItem);
    }
  }
};
